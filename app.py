...
def load_and_process_data():
    ...
    try:
        ...
        # Calculate derived metrics for marketing data (keep original column names)
        ...
        # Calculate additional business metrics (using original column names)
        combined_df['gross margin %'] = np.where(combined_df['total revenue'] > 0, 
                                               (combined_df['gross profit'] / combined_df['total revenue']) * 100, 0)
        ...
        combined_df = combined_df.merge(total_daily_marketing, on='date', suffixes=('', '_total'))
        combined_df['spend_ratio'] = np.where(combined_df['spend_total'] > 0,
                                            combined_df['spend'] / combined_df['spend_total'], 0)
        
        combined_df['estimated_new_customers'] = combined_df['spend_ratio'] * combined_df['new customers']
...
def create_kpi_cards(data, date_filter):
    ...
    avg_gross_margin = filtered_data['gross margin %'].mean()
...
def create_trend_charts(data, date_filter):
    ...
    with col2:
        ...
        business_trends = filtered_data.groupby('date').agg({
            'spend': 'sum',
            'new customers': 'first'  # Since it's the same for all platforms on same date
        }).reset_index()
        ...
        fig.add_trace(
            go.Scatter(x=business_trends['date'], y=business_trends['new customers'], 
...
def create_marketing_funnel(data, date_filter):
    ...
    with col1:
        ...
        for i, platform in enumerate(platforms):
            ...
            orders = platform_data['# of orders'].iloc[0] if not pd.isna(platform_data['# of orders'].iloc[0]) else 0
            revenue = platform_data['attributed revenue'].iloc[0]
            
            # Create funnel stages
            stages = ['Impressions', 'Clicks', 'Orders', 'Revenue (₹)']
            values = [impressions, clicks, orders * 1000, revenue]  # Scale orders for visualization
...
def generate_insights(data, date_filter):
    ...
    with col1:
        ...
        st.markdown(f"""
        <div style="background-color:#d4edda; padding:12px; border-radius:8px; color:black;">
        ...
        if not poor_roas_platforms.empty:
            for _, platform in poor_roas_platforms.iterrows():
                st.markdown(f"""
                <div style="background-color:#f8d7da; padding:12px; border-radius:8px; color:black;">
    ...
    with col2:
        ...
        if len(platform_metrics) > 1:
            ...
            if best_platform['ROAS'] > worst_platform['ROAS'] * 1.5:
                st.markdown(f"""
                <div style="background-color:#fff3cd; padding:12px; border-radius:8px; color:black;">
